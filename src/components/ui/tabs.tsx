...
>(({ className, ...props }, ref) => (
  <TabsPrimitive.List
    ref={ref}
    className={cn(
      // max-w-full + overflow-x-auto so a strip that outgrows a phone screen scrolls inside
      // itself instead of forcing page-level horizontal overflow — the same convention already
      // applied to this app's wide dashboard tables. `auto` means no scrollbar appears when the
      // tabs fit, so desktop is visually unchanged.
